...
//
//    let url = "https://aitradingstation.com"
//
//    // MARK: - Apple Sign In
//
...
//
//        guard timeUntilExpiry > 0 else {
//            print("⚠️ Access token already expired — renewing now.")
//            Task { await renewAccessToken() }
//            return
//        }
//
...
//                let remainingTime = expiryDate.timeIntervalSinceNow
//
//                // ⚠️ Trigger early refresh when near expiry
//                if remainingTime <= self.warningThreshold, !self.hasTriggeredRefresh, remainingTime > 0 {
//                    self.hasTriggeredRefresh = true
//                    print("⚠️ Access token expiring soon (\(Int(remainingTime))s left). Renewing now...")
//                    await self.renewAccessToken()
//                }
//
//                // ⏰ Token expired — force immediate refresh
//                if remainingTime <= 0 {
//                    print("⏰ Access token expired. Auto-renewing...")
//                    self.stopTimer()
//                    await self.renewAccessToken()
//                }
//            }
...
//        let timeUntilExpiry = expiryDate.timeIntervalSince(now)
//
//        // 🩵 FIX: Handle token already expired when app opens
//        guard timeUntilExpiry > 0 else {
//            print("⚠️ Refresh token already expired — renewing now.")
//            Task {
//                await renewRefreshToken()
//                await MainActor.run {
//                    self.startRefreshTokenExpiryTimer() // restart timer with new expiry
//                }
//            }
//            return
//        }
...
//            await renewRefreshToken()
//            await MainActor.run {
//                self.startRefreshTokenExpiryTimer() // 🩵 restart after renewal
//            }
//        }
//    }
//
...
//    // MARK: - Delete User
//    
//    @Published var isShowingDeleteUser: Bool = false
//    
//    func deleteUser() async {
//        guard let accessToken = loadFromKeychain(key: "access_token"),
//              let url = URL(string: "\(url)/api/user") else {
//            serverErrorMessage = "Invalid URL"
//            return
//        }
//
//        serverErrorMessage = nil
...
//                if let serverError = try? JSONDecoder().decode(ServerErrorResponse.self, from: data) {
//                    serverErrorMessage = serverError.detail
//                    print(serverErrorMessage as Any)
//                } else {
//                    let statusText = HTTPURLResponse.localizedString(forStatusCode: httpResponse.statusCode).capitalized
//                    serverErrorMessage = "Delete failed: \(httpResponse.statusCode) \(statusText)"
//                    print(serverErrorMessage as Any)
//                }
//            }
//
//        } catch {
//            serverErrorMessage = "Delete request failed: \(error.localizedDescription)"
...
//        if let accessToken = loadFromKeychain(key: "access_token") {
